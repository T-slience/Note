...
# 6.静态Web服务器--动态端口
    - 1.步骤分析
        - 在之前的服务器搭建中,都是指定了端口执行的
        - 这里需要导入sys模块,用到sys中的argv
        - sys.argv可以获得终端上输入的参数,以列表的形式,且内容均为字符串
        - 故只需要在之前的搭建服务器中将端口改为某个实参,在对其传参即可实现动态端口
...
def main():
    ...
    # 判断终端上是否输入了两个参数
    if len(sys.argv) != 2:
        print("请按以下格式填写: python3 xxx.py 9000")
        return
    # 判断第二个参数是否是由数字组成,因为其代表端口号
    if not sys.argv[1].isdigit:
        print("请按以下格式填写: python3 xxx.py 9000")
        return
    # 都符合的话,将其转换成int 类型并赋值给形参
    port = int(sys.argv[1])
    # 创建web服务器对象
